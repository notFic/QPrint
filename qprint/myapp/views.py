...
# --------------------------
def check_session_timeout(get_response):
    def middleware(request):
        if request.user.is_authenticated:
            last_activity = request.session.get('last_activity')
            now = timezone.now().timestamp()
            if last_activity and now - last_activity > 900:  # 15 minutes
                auth_logout(request)
...
def register(request):
    if request.method == "POST":
        ...
        agreed_terms = request.POST.get("agree_terms")

        if not agreed_terms:
            messages.error(request, "You must agree to the Terms and Policies to register.")
...
def verify(request):
    ...
    if request.method == "POST":
        ...
        if otp_created_ts and time.time() - otp_created_ts > OTP_EXPIRE_SECONDS:
        ...
        if entered_otp == otp:
            ...
            agreed_terms = request.session.get("agreed_terms", False)
            ...
            for k in ("username", "email", "password", "otp", "otp_last_sent", "otp_created", "agreed_terms"):
...
def login(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            try:
                supabase = get_supabase()
                supabase.table('login_activity').insert({'user_id': user.id, 'login_time': 'now()'}).execute()
            except Exception as e:
...
def logout(request):
    if request.user.is_authenticated:
        try:
            supabase = get_supabase()
            supabase.table('logout_activity').insert({'user_id': request.user.id, 'logout_time': 'now()'}).execute()
        except Exception as e:
            print(f"Supabase activity logging error: {e}")
...
def terms_and_policies(request):
    return render(request, "myapp/terms_and_policies.html")

def session_expired(request):
    return render(request, "myapp/session_expired.html")